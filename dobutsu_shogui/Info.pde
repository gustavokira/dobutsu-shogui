class Info{
  ...
  public Info(Jogo jogo, JogadorCore j){
    ...
    print(this.meuLeaoEstaSendoAtacado);
    
      //for(int k = 0;k<matrizDeAtaque[0].length;k++){
      //  for(int i = 0;i<matrizDeAtaque.length;i++){
      //    print(matrizDeAtaque[i][k]," ");
      //}
//    }
        ...
        if( !(x < 0 || x > this.tabuleiro.getLargura()-1 || y< 0 || y > this.tabuleiro.getAltura()-1)){
          ...
          if(p.getNome().equals("leo")){
              if(!matrizDeAtaque[x][y]){
                Movimento m = new Movimento(p,x,y,"mover");
    ...
    for(Peca p :pecasMinhas){
      for(Casa c:casasVazias){
        Movimento m = new Movimento(p,c.getX(),c.getY(),"colocar");
        this.movimentos.add(m);
      }
    }
