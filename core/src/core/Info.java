...
public class Info {
	...
	// para criar um objeto Info, é preciso ter o estado atual do jogo e quem é
	// o jogador que deve fazer um movimento.
		...
		for (Peca p : pecasMinhas) {
			...
			for (int i = 0; i < matriz.length; i++) {
				...
				if (!(x < 0 || x > this.tabuleiro.getLargura() - 1 || y < 0 || y > this.tabuleiro.getAltura() - 1)) {
					...
					// se for o leao, não pode mover onde o oponente esta
					// atacando!
					if (p.getNome().equals("leo")) {
